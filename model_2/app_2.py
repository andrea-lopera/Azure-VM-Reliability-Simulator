...
model = joblib.load('model_2/model.pkl')
ctv = CoxTimeVaryingFitter()
ctv = joblib.load('model_2/survival_model_1.pkl')
try:
    explainer = joblib.load('model_2/shap_explainer_1.joblib')
except:
    with open('model_2/shap_explainer_1.pkl', 'rb') as f:
        explainer = pickle.load(f) 
df = pd.read_csv('model_2/vm_telemetry_2.csv')
failure_history = np.load('model_2/failure_history_2.npy', allow_pickle=True).item()
with open('model_2/feature_names_2.txt', 'r') as f:
...
st.title("Azure VM Reliability Simulator 2")
...
st.dataframe(df[df['vm_id'] == vm_id].tail(10)[['cumulative_failures', 'cpu_util', 'mem_util', 'disk_io', 'net_latency', 'cpu_util_7d_avg', 'disk_io_7d_avg']])
